...
export default function CompactArticleCard({
    ...
    const imageSizes = {
        default: 'relative w-full h-32',
        horizontal: 'relative w-20 h-20 flex-shrink-0',
        minimal: 'hidden'
    };

    const imageSizesProp = {
        default: '(max-width: 768px) 100vw, 300px',
        horizontal: '80px',
        minimal: '80px',
    };
    ...
    return (
        <Link href={`/blogs/${slug}`} className={`group block ${variantStyles[variant]}`}>
            {/* Image */}
            {showImage && featured_image && variant !== 'minimal' && (
                <div className={imageSizes[variant]}>
                    <Image
                        src={featured_image}
                        alt={title}
                        fill={variant === 'horizontal'}
                        width={variant === 'default' ? 300 : undefined}
                        height={variant === 'default' ? 128 : undefined}
                        sizes={imageSizesProp[variant]}
                        ...
                        {like_count !== undefined && like_count > 0 && (
                            <>
                                <span className="flex items-center gap-1">
                                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                                            d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0h0z" />
                                    </svg>
                                    {like_count}
                                </span>
                                <span>•</span>
